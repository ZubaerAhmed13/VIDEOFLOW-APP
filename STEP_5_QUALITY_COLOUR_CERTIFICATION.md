...
Resource measurements separately record descriptors before/after each production render and after test-only frame extraction. The production-render growth gate remains below 20 across the repeated exports. fd-phases.jsonl retains all phase counts, including emulator frame-reader overhead; physical GPU/codec resource endurance remains NOT VERIFIED.

Step5CompositionGeometryTest checks background coverage, aspect-preserving fit at a different output resolution/aspect ratio, and text layer order/size using decoded pixels. The flash detector samples the original center pixel, retaining sensitivity to the fixed opaque clock-square defect. Measurements are in composition-geometry.jsonl.
