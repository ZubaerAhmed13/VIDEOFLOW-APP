...
The repeated-render gate exposed missing explicit placeholder-surface cleanup in Media3 1.11.0's FinalShaderProgramWrapper. A narrowly scoped AGP ASM visitor destroys each processor's owned placeholder inside the existing GL exception handling and releases the shared graph worker's EGL thread state after context destruction. It also makes DefaultVideoCompositor select the latest secondary frame at or before each CFR output timestamp, preventing a future frame from appearing early. The existing queue release logic retains the preceding frame and remains unchanged. All three exact class/method corrections fail closed if their expected bytecode boundary changes. CI disassembles both packaged app APKs and verifies the actual calls. The repeated-render descriptor gate and live-preview-after-export product check then validate runtime behavior. The pinned Media3 version remains unchanged; these compatibility corrections require review before any future Media3 upgrade.

SAF MP4 muxing uses the pinned Media3 Mp4Muxer on an AutoCloseOutputStream backed by the selected seekable descriptor. Sample batching and streamable-output reservation are disabled: encoded payloads are written immediately and no full-size intermediate copy is created. Video sorts before audio independently of encoder initialization order. Signed AAC priming timestamps are retained in MP4 edit lists instead of losing encoder-delay compensation at the container boundary. Rotation and supported container metadata are retained. Android decoder compatibility is checked with sequential image/PCM decoding.
