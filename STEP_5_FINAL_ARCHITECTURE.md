...
The repeated-render gate exposed retained EGL worker state in Media3 1.11.0's `MultipleInputVideoGraph.SingleContextGlObjectsProvider.release`. A narrowly scoped AGP ASM visitor adds `GlUtil.terminate(eglDisplay)` after the existing owned-context destruction, matching Media3's default provider by releasing both worker state and the display initialization reference. It targets only that exact class/method, fails if the method changes, and applies to every app variant. This does not replace the compositor or alter rendered pixels. Reference: https://github.com/androidx/media/blob/1.11.0/libraries/effect/src/main/java/androidx/media3/effect/MultipleInputVideoGraph.java. The resource gate must validate the correction on the exact APK; the product gate keeps an editor preview alive during export and requires new visible preview frames afterward.
