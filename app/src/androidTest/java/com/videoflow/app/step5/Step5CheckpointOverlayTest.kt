...
class Step5CheckpointOverlayTest {
    @Test
    fun checkpointedVideoAssemblesWithContinuousOriginalAudio() = runBlocking {
        ...
        val projectId = "step5-overlay-checkpoint-$suffix"
        ...
        try {
            ...
            val plan = FinalRenderPlan(
                editorPlan = RenderPlan(
                    ...
                    tracks = listOf(track,TimelineTrack("overlay",projectId,TrackType.OVERLAY,"Titles",1)),
                    clips = listOf(clip),
                    textOverlays = listOf(com.videoflow.app.domain.editor.TextOverlay("title",projectId,"overlay",500_000L,1_500_000L,"STEP 5",fontSizeSp=24f)),
            ...
            val resumedEngine = com.videoflow.app.render.SegmentedAiRenderEngine(context, Media3RenderEngine(context, aiRepository, manager), aiRepository).apply { checkpointIntervalUs = 1_000_000L }
            val result = resumedEngine.render(requireNotNull(prepared.preparation), com.videoflow.app.render.RenderProgressListener {}).getOrThrow()
            assertTrue("Resume must reuse at least one completed segment", resumedEngine.resumedSegmentCount >= 1L)
            ...
            val marker =
                "STEP5_OVERLAY_CHECKPOINT_CERTIFIED project=$projectId bytes=${result.outputBytes} " +
