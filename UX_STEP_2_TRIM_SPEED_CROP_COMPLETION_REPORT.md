...
- Software implementation SHA before final certification metadata: `68652f46ac2389e86a4dbed2592ae7552d6e8c52`
...
## Crop
### Previous stretching root cause
The Compose preview divided X scale by crop width and Y scale by crop height independently. For non-source-aspect crops, those factors differed and visibly deformed people and objects.

### Corrected transform model
The preview now uses one uniform crop scale from `uniformCropPreviewGeometry(...)`; X and Y receive the same crop scale. Crop positioning is handled by source-content-aware translation rather than non-uniform stretching. During active Crop editing the full undistorted source is shown beneath the normalized crop overlay.
...
The existing Media3 final-render path still applies a real `Crop` effect followed by uniform `aspectFitScale()`. Final export remains original-source/native-render based; no bitmap/screenshot export path, forced 720p downgrade, or independent X/Y crop scaling was introduced.
...
Production changes are confined to Trim/Speed/Crop editor state, UI, preview geometry, repository/domain helpers, tests, this report and the dedicated Step-2 certification workflow. Step-2 source-scope certification rejects changes under the AI and proxy implementation directories.
