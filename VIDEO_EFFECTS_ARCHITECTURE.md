...
Parameter changes update an atomic immutable stage snapshot read by the existing shader, then redraw the cached paused frame. This avoids clearing the cache or rebuilding a decoder on every slider tick. Structural changes (adding/removing/reordering stages) reprepare the same player at its retained position because Media3 clears replay frames when registering a changed effect graph. The UI gate compares visible surface pixels against a stable paused baseline for both stage addition and parameter adjustment.
